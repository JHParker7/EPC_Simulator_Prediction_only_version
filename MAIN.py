...
def preproccesing(Data):
    Data=Data[["property-type","built-form","local-authority","transaction-type","total-floor-area","energy-tariff","mains-gas-flag","floor-level","flat-top-storey","multi-glaze-proportion",
    "glazed-type","glazed-area","extension-count","number-habitable-rooms","number-heated-rooms","low-energy-lighting","number-open-fireplaces","hotwater-description","floor-description","windows-description",
    "walls-description","roof-description","mainheat-description","mainheatcont-description","main-fuel","heat-loss-corridor","unheated-corridor-length","floor-height","solar-water-heating-flag","mechanical-ventilation",
    "construction-age-band","fixed-lighting-outlets-count","low-energy-fixed-light-count","tenure","secondheat-description"]]
    Data=Data.rename(columns={"Unnamed: 0":"temp_index"})
    Data=Data.reset_index(drop=True)
    Data=Data.replace("Very Poor",1).replace("Poor",2).replace("Average",3).replace("Good",4).replace("Very Good",5).replace("G",1).replace("F",2).replace("E",3).replace("D",4).replace("C",5).replace("B",6).replace("A",7)
    Data["property-type"]=Data["property-type"].replace("Flat",1).replace("House",2).replace("Bungalow",3).replace("Maisonette",4).replace("Park home",5)
    ...
    Data["local-authority"]=[x.split("E")[-1].split("W")[-1] if isinstance(x,str) else x for x in Data["local-authority"].tolist()]
    Data["local-authority"]=Data["local-authority"].astype(float)
    Data=Data.replace('NO DATA!',np.nan)
    Data=Data.replace([np.inf, -np.inf], np.nan)
    Data=Data[["property-type","built-form","local-authority","transaction-type","total-floor-area","energy-tariff","mains-gas-flag","floor-level","flat-top-storey","multi-glaze-proportion",
    "glazed-type","glazed-area","extension-count","number-habitable-rooms","number-heated-rooms","low-energy-lighting","number-open-fireplaces","hotwater-description","floor-description","windows-description",
    "mainheat-description","mainheatcont-description","main-fuel","heat-loss-corridor","unheated-corridor-length","floor-height","solar-water-heating-flag","mechanical-ventilation",
    "construction-age-band","fixed-lighting-outlets-count","low-energy-fixed-light-count","tenure","secondheat-description","insulation-wall","wall type","Average thermal transmittance-wall","insulation-roof",
    "roof type","Average thermal transmittance-roof"]]
    warnings.filterwarnings('ignore')
    ...
    Data.columns=["property-type","built-form","local-authority","transaction-type","total-floor-area","energy-tariff","mains-gas-flag","floor-level","flat-top-storey","multi-glaze-proportion",
    "glazed-type","glazed-area","extension-count","number-habitable-rooms","number-heated-rooms","low-energy-lighting","number-open-fireplaces","hotwater-description","floor-description","windows-description",
    "mainheat-description","mainheatcont-description","main-fuel","heat-loss-corridor","unheated-corridor-length","floor-height","solar-water-heating-flag","mechanical-ventilation",
    "construction-age-band","fixed-lighting-outlets-count","low-energy-fixed-light-count","tenure","secondheat-description","insulation-wall","wall type","Average thermal transmittance-wall","insulation-roof",
    "roof type","Average thermal transmittance-roof"]
...
def TEST(Data,Y):
    ...
    print(Y)
    print(PRE)
    for y in ["HEATING_COST_CURRENT","HOT_WATER_COST_CURRENT","ENVIRONMENT_IMPACT_CURRENT","ENERGY_CONSUMPTION_CURRENT","CO2_EMISSIONS_CURRENT","LIGHTING_COST_CURRENT"]:
        print(y)
...
#TEST(DF,Y)
DF[["PRE_HEATING_COST_CURRENT","PRE_HOT_WATER_COST_CURRENT","PRE_ENVIRONMENT_IMPACT_CURRENT","PRE_ENERGY_CONSUMPTION_CURRENT","PRE_CO2_EMISSIONS_CURRENT","PRE_LIGHTING_COST_CURRENT"]]=CORE(DF)
